...
public class SubscribeWriteController {
	...
	public BaseResponse<Void> createSubscribe(@RequestBody SubscribeRequestVo subscribeRequestVo) {
		subscribeService.createSubscribe(SubscribeRequestDto.toDto(subscribeRequestVo));
		return new BaseResponse<>();
	}

	@Operation(summary = "게시글 알림 신청 취소 API", description = "해당 게시글의 알림을 취소합니다.", tags = {"Subscribe"})
	@DeleteMapping("/{authorUuid}")
	public BaseResponse<Void> deleteSubscribe(@RequestBody SubscribeRequestVo subscribeRequestVo) {
		subscribeService.deleteSubscribe(SubscribeRequestDto.toDto(subscribeRequestVo));
		return new BaseResponse<>();
	}
